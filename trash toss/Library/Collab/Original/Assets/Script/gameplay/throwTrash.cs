...
//		} else {
		...
		if (lastMousePosition != newMousePosition) {
			...
			moveBySwipe = true;
			startCounting = true;
		} else if (TapCount == 0 && gameObject.CompareTag("doubleTap")) {
			TapCount++;
	...
	public void returnToOrigin(GameObject itemToReset)
	{
		moveByBelt = true;
		moveBySwipe = false;
	...
	public void handle2step()
	{
		for(int i = 0; i < gameObject.transform.childCount; i++)
		{
			Transform newChild = Instantiate(gameObject.transform.GetChild(i), transform.position, transform.rotation);

			newChild.gameObject.active = true;
			newChild.gameObject.GetComponent<throwTrash>().returnToOrigin( newChild.gameObject);
		}
		//Destroy(gameObject);
	}
	...
	public GameObject raycastForBin(Vector3 origin, Vector3 direction){
		...
		for (int rayDistance = 0; rayDistance < 100; rayDistance += rayIncrement){
			...
			float distanceFromBinToRayPoint = Vector3.Distance(sink.gameObject.transform.position , pointToCheck);
